...
function proc5() {
    sum = Math.ceil((Number(input1.value) / 5 / Number(input2.value)));
...
function proc10() {
    sum = Math.ceil((Number(input1.value) / 10 / Number(input2.value)));
    res.textContent = "Сумма на каждого по: " + sum + " грн";
    showTime();
}

function proc15() {
    sum = Math.ceil((Number(input1.value) / 15 / Number(input2.value)));
    res.textContent = "Сумма на каждого по: " + sum + " грн";
    showTime();
}

function proc20() {
    sum = Math.ceil((Number(input1.value) / 20 / Number(input2.value)));
    res.textContent = "Сумма на каждого по: " + sum + " грн";
    showTime();
}
